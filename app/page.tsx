...
const HomePage = (props: Props) => {
	...
	async function runPrediction() {
		if (
			...
			const newPersonCount = predictions.filter(
				(prediction) => prediction.class === "person"
			).length;
			if (newPersonCount > 1) {
				// Beep sound when a new person enters the camera
				beep(volume);
				startRecording(true)
			}
			setPersonCount(newPersonCount);
			let isPerson: boolean = newPersonCount > 0;
		...
		<div className="flex  h-screen">
			<div className="relative">
				<div className="relative h-screen w-full">
					...
					<canvas
						ref={canvasRef}
						className="absolute h-full w-full left-0 top-0 object-contain"
					></canvas>
				</div>
			</div>
			<div className="flex flex-row flex-1">
				<div className="border-primary/5 border-2 flex flex-col gap-2 justify-between shadow-md rounded-md p-4">
					{/* top-section */}
	...
	function userPromptScreenshot() {
		...
		if (!webcamRef.current) {
			...
			const a = document.createElement("a");
			a.href = url;
			a.download = `img ${formatDate(new Date())}.png`;
			a.click();
		}
		...
		return (
			<div className="text-xs text-muted-foreground">
				<ul className="space-y-4">
					<li>
						<strong>Light Mode/Dark Mode/System Theme 🌗</strong>
						<p>Toggle between dark mode and light theme.</p>
					...
					<li>
						...
						<p>
							Option to enable/disable automatic video recording when one person enters
						</p>
						<Button
							className="h-6 w-6 my-2"
							variant={autoRecordEnabled ? "destructive" : "outline"}
